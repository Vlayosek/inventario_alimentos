...
namespace App\Http\Controllers\InventarioAlimentos\Carondelet;

use App\Http\Controllers\InventarioAlimentos\RepositorioController;
...
class ProductoController extends Controller
{
  ...
  public function datatableCocinaProductosPOSTServerSide(Request $request)
  {
    $data = Producto::select(
      ...
      ->addSelect([
        'stock_' =>
        Transaccion::select('transacciones.stock_disponible')
          ->whereColumn('transacciones.producto_id', 'productos.id')
          ->where('transacciones.tipo_transaccion', 'COMPRA')
          ->where('transacciones.estado', 'ACT')
          ->where('transacciones.ultima_transaccion', true)
      ])
      ->addSelect([
        'compras_' =>
        Transaccion::select(DB::RAW("SUM(transacciones.cantidad)"))
          ->whereColumn('transacciones.producto_id', 'productos.id')
          ->where('transacciones.tipo_transaccion', 'COMPRA')
      ])
      ->addSelect([
        ...
        Transaccion::select(DB::RAW("SUM(transacciones.cantidad)"))
          ->whereColumn('transacciones.producto_id', 'productos.id')
          ->where('transacciones.tipo_transaccion', 'SALIDA')
      ])
      ->addSelect([
        'devoluciones_' =>
        Transaccion::select(DB::RAW("SUM(transacciones.cantidad)"))
          ->whereColumn('transacciones.producto_id', 'productos.id')
          ->where('transacciones.tipo_transaccion', 'DEVOLUCION')
      ])
